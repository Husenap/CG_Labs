...
edaf80::Assignment5::Assignment5(WindowManager& windowManager)
    : mCamera(0.5f * glm::half_pi<float>(), static_cast<float>(RESOLUTION_X) / static_cast<float>(RESOLUTION_Y), 0.01f, 1000.0f),
...
void edaf80::Assignment5::run() {
  ...
  program_manager.CreateAndRegisterProgram(
      "Default", {{ShaderType::vertex, "EDAF80/default.vert"}, {ShaderType::fragment, "EDAF80/default.frag"}}, default_shader);
  GLuint screen_shader = 0u;
  program_manager.CreateAndRegisterProgram(
      "Screen", {{ShaderType::vertex, "EDAF80/default.vert"}, {ShaderType::fragment, "EDAF80/screen.frag"}}, screen_shader);
  ...
  GLuint screen_texture = bonobo::createTexture(FBW, FBH, GL_TEXTURE_2D, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, framebuffer);
  ...
  const auto set_screen_uniforms = [&](GLuint program) { glUniform1f(glGetUniformLocation(program, "time"), elapsed_time_s); };
  ...
  while (!glfwWindowShouldClose(window)) {
      ...
      if (ptr) {
        ...
        for (int b = 0; b < 5; ++b) {
          ...
          if (fb) {
            ...
            for (int y = 0; y < 2; ++y) {
              for (int x = 0; x < 4; ++x) {
                const int i = (FBH - (BOARD_OFFSET_Y + y + b * 4)) * FBW + (x + BOARD_OFFSET_X + BOARD_WIDTH + 1);
                ptr[i]      = (y < h && x < w) ? get_color(fb[y * w + x]) : COLOR_BLACK;
              }
            }
          }
    ...
    if (!shader_reload_failed) {
      ...
      const float  target_angle = (static_cast<float>(key_input.left) - static_cast<float>(key_input.right)) * 0.4f;
      ...
      joystick.render(mCamera.GetWorldToClipMatrix(), trans * rotX * rotZ);
